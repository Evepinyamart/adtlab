...
tree_t *init_tree(int fre, char *word)
{
...
void insert(heap_t *h, tree_t *tree)
{
    ...
    while (i > 1)
    {
        if (h->data[i]->fre < h->data[i / 2]->fre)
        {
...
tree_t *delete_min(heap_t *h)
{
    ...
    {
        if (h->data[i]->fre > h->data[i * 2]->fre || h->data[i]->fre > h->data[i * 2 + 1]->fre)
        {
            if (h->data[i * 2]->fre <= h->data[i * 2 + 1]->fre)
            {
                swap(&h->data[i], &h->data[i * 2]);
                i *= 2;
            }
